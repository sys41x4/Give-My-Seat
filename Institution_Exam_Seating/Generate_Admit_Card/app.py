...
def home():
	...
	# #preview_admit_card=roll_num

 # #    std_name = std_seating_info['std_name']
	# # branch_name = std_seating_info['branch_name']
	# # roll_num = std_seating_info['roll_num']
	# # aadhar_num = std_seating_info['aadhar_num']
	# # block_num = std_seating_info['block_num']
	# # room_num = std_seating_info['room_num']
	# # row = std_seating_info['row']
	# # column = std_seating_info['column']
	# # base64_qr_str = '<img src=\"data:image/png;base64,'+gen_verifiable_qr_code(std_seating_info)+'\"/>'

	# # std_seating_info.update({'base64_qr_str':base64_qr_str, 'institute_name':institute_name})

	return jsonify({"roll_num":roll_num})
	# return f'''<iframe src="/{roll_num}"></iframe>'''
...
def generate_pdf(std_seating_info):
	...
	pdf = pdfkit.from_string(rendered, False, configuration=pdfkit.configuration(wkhtmltopdf='wkhtmltox\\bin\\wkhtmltopdf.exe'))
...
def admitcard(aadhar_num, roll_num):
	...
	std_seating_info = fetch_json_data(confirmed_seating_data_path, roll_num)
	# JSON Data
	# {"std_name": student_name,"branch_name": branch_name,"roll_num": roll_num,"aadhar_num": aadhar_num,"block_num": current_block,"room_num": current_room,"row": row_num,"column": col_num }
	# std_name = std_seating_info['std_name']
	# branch_name = std_seating_info['branch_name']
	# roll_num = std_seating_info['roll_num']
	# aadhar_num = std_seating_info['aadhar_num']
